...
class ConnectionManager:
    def __init__(self) -> None:
        self.station_connections: dict[UUID, set[WebSocket]] = defaultdict(set)
        self.order_connections: dict[UUID, set[WebSocket]] = defaultdict(set)
        self.menu_connections: set[WebSocket] = set()

    async def connect_station(self, station_id: UUID, websocket: WebSocket) -> None:
    ...
    def disconnect_station(self, station_id: UUID, websocket: WebSocket) -> None:
    ...
    async def broadcast_station(self, station_id: UUID, envelope: WsEnvelope) -> None:
        for websocket in list(self.station_connections[station_id]):
            await websocket.send_json(envelope.model_dump(mode="json"))

    async def broadcast_order(self, order_id: UUID, envelope: WsEnvelope) -> None:
...
class WebSocketBroadcaster:
    ...
    async def emit_station(self, station_id: UUID, message_type: str, data: dict) -> None:
        await self.manager.broadcast_station(
            station_id, WsEnvelope(type=message_type, data=data)
        )

    async def emit_order(self, order_id: UUID, message_type: str, data: dict) -> None:
